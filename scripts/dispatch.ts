...
const verifyReleaseGateCmd = defineCommand({
  meta: {
    name: 'verify',
    description: 'Verify the release gate (proof manifest checks)'
  },
  run() {
    const { verifyReleaseGate } = require('./verify-release-gate');
    verifyReleaseGate();
  }
});
...
const mainCommand = defineCommand({
  meta: {
    name: 'dispatch',
    version: '1.0.0',
    description: 'Truex Dev CLI - dispatch command parser'
  },
  subCommands: {
    verify: verifyReleaseGateCmd,
