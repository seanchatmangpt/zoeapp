...
describe('SyncEngine Outbox System', () => {
  ...
  describe('pushChanges', () => {
    ...
    it('should queue a rerun of pushChanges via needsPush if pushChanges is called during execution', async () => {
      ...
      mockedDb._mockOrderBy.mockResolvedValueOnce([job]); // Run 1 loop 1
      mockedDb._mockOrderBy.mockResolvedValueOnce([]);    // Run 1 loop 2
      mockedDb._mockOrderBy.mockResolvedValueOnce([job]); // Run 2 loop 1
      mockedDb._mockOrderBy.mockResolvedValueOnce([]);    // Run 2 loop 2

      engine.mockDispatchJob.mockImplementationOnce(async () => {
        // Trigger concurrent push
        await engine.pushChanges();
      });
    ...
    it('should skip jobs for an entity if there is a quarantined or processing job for that entity', async () => {
      ...
      const jobB: SyncJob = {
        ...
        status: 'pending',
        attempts: 0,
        entityId: 'free_entity',
        createdAt: new Date(),
      };

      // Mock blocked jobs query to return that 'blocked_entity' has a blocked job
      mockedDb._mockWhereBlocked.mockResolvedValueOnce([{ entityId: 'blocked_entity' }]);
      ...
      // Only jobB (free_entity) should be processed, jobA1 should be skipped because its entity is blocked
      expect(engine.mockDispatchJob).toHaveBeenCalledTimes(1);
    ...
    it('should filter by supportedJobTypes if defined in the subclass', async () => {
      class TypeScopedSyncEngine extends SyncEngine {
        protected supportedJobTypes = ['TYPE_A'];
        protected async dispatchJob(job: SyncJob): Promise<void> {}
      }
